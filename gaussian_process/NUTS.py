# Adapted from mfouesneau github

import numpy as np
from numpy import log, exp, sqrt
from optimizers import velo_integrate
...
def find_reasonable_epsilon(theta0, grad0, logp0, f):
    ...
    _, rprime, nlogpprime, gradprime = velo_integrate(theta0, r0, grad0, f, 1, dt = epsilon, steps = 200)
    logpprime = -nlogpprime
    ...
    while np.isinf(logpprime) or np.isinf(gradprime).any():
        k *= 0.5
        _, rprime, nlogpprime, _ = velo_integrate(theta0, r0, grad0, f, 1, dt = epsilon * k, steps = 200)
        logpprime = -nlogpprime
    ...
    while a * logacceptprob > -a * np.log(2):
        epsilon = epsilon * (2. ** a)
        _, rprime, nlogpprime, _ = velo_integrate(theta0, r0, grad0, f, 1, dt = epsilon, steps = 200)
        logpprime = -nlogpprime
        # acceptprob = np.exp(logpprime - logp0 - 0.5 * ( np.dot(rprime, rprime.T) - np.dot(r0, r0.T)))
        logacceptprob = logpprime-logp0-0.5*(np.dot(rprime, rprime)-np.dot(r0,r0))
...
def build_tree(theta, r, grad, logu, v, j, epsilon, f, joint0):
    ...
    if (j == 0):
        ...
        thetaprime, rprime, nlogpprime, gradprime = velo_integrate(theta, r, grad, f, 1, dt = epsilon, steps = 200)
        logpprime = -nlogpprime
...
def nuts6(theta0, f, M, Madapt, delta=0.6):
    """
