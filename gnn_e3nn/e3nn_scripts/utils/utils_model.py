...
class PeriodicNetwork(Network):
    ...
    def forward(self, data: Union[tg.data.Data, Dict[str, torch.Tensor]]) -> torch.Tensor:
        # Leaky ReLU to deal with negative values
        data.x = F.leaky_relu(self.em(data.x))
        data.z = F.leaky_relu(self.em(data.z))
...
def evaluate(model, dataloader, loss_fn, loss_fn_mae, device):
    ...
    with torch.no_grad():
        for j, d in enumerate(dataloader):
            ...
            #     print("nan output in evaluate")
            #     continue
            # else:
            #     print("no nan output in evaluate")
            loss = loss_fn(output, d.hform).cpu()
            loss_mae = loss_fn_mae(output, d.hform).cpu()
...
def train(model, optimizer, dataloader_train, dataloader_valid, loss_fn, loss_fn_mae, run_name,
          cwd, max_iter=101, scheduler=None, device="cpu"):
    model.to(device)
    ...
    for step in range(max_iter):
        ...
        for j, d in tqdm(enumerate(dataloader_train), total=len(dataloader_train), bar_format=bar_format):
            ...
            output = model(d)
            # if torch.isnan(output).any():
            #     print("nan output", d.form, d.hform)
            #     avoids.append(d.form)
            #     continue
            # else:
            #     print("no nan output", d.form, d.hform)
            # print(output, d.hform)
            loss = loss_fn(output, d.hform).cpu()
            loss_mae = loss_fn_mae(output, d.hform).cpu()
            loss_cumulative = loss_cumulative + loss.detach().item()
            loss_cumulative_mae = loss_cumulative_mae + loss_mae.detach().item()
        ...
        with open(f'{cwd}/{run_name}.torch', 'wb') as f:
            torch.save(results, f)
        
        with open(f'{cwd}/{run_name}_full_model.torch', 'wb') as f:
            torch.save(model, f)
